...
internal static class ValueConversion
{
    ...
    public static bool IsSupported(Type type)
    {
        ...
        return target == typeof(string)
            ...
            || target == typeof(DateTime) || target == typeof(DateOnly) || target == typeof(TimeOnly)
    ...
    public static bool TryConvert(string text, Type type, bool excelSerialDates, out object? value, out string? error)
    {
        ...
        if (target == typeof(DateTime) || target == typeof(DateOnly) || target == typeof(DateTimeOffset))
        {
            if (TryParseDateTime(trimmed, excelSerialDates, out var date))
            {
                // Separate assignments on purpose: a conditional expression here would give all three branches one type
                // (DateTimeOffset) and box a plain DateTime as the wrong type.
        ...
        if (target == typeof(TimeOnly) || target == typeof(TimeSpan))
        {
            if (excelSerialDates && double.TryParse(trimmed, NumberStyles.Float, CultureInfo.InvariantCulture, out var days))
            {
                ...
                value = target == typeof(TimeOnly) ? TimeOnly.FromTimeSpan(TimeSpan.FromTicks(span.Ticks % TimeSpan.TicksPerDay)) : span;
                return true;
            }

            if (target == typeof(TimeOnly) && TimeOnly.TryParse(trimmed, CultureInfo.InvariantCulture, out var time))
            {
                value = time;
                return true;
            }
        ...
        if (target.IsEnum)
        {
            // A number is accepted only when the enum defines it. Enum.TryParse would accept any number.
            if (Enum.TryParse(target, trimmed, ignoreCase: true, out var member) && Enum.IsDefined(target, member!))
            {
                value = member;
                return true;
            }
